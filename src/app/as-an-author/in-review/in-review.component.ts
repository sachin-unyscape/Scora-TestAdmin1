...
export class InReviewComponent implements OnInit {
  ...
  ItemSetList(){
    ...
    var headers = new Headers();
    headers.append('Authorization', 'Bearer ' + this.cookieService.get('_PTBA'));
    this.http.get(credentials.host +"/review_menu/" + this.cookieService.get('_PAOID') +'/' + 'under_review',{headers:headers})
    .map(res => res.json())
    .catch((e: any) =>{
      return Observable.throw(e)
    } )
  ...
  getItemsetDet(id,sectionid){
    if(this.authService.canActivate()){
      ...
      var headers = new Headers();
      headers.append('Authorization', 'Bearer ' + this.cookieService.get('_PTBA'));
      this.http.get(credentials.host + "/item_details/" +this.cookieService.get('_PAOID') +"/"+ id + "/all_sections/" + 3,{headers:headers})
      .map(res => res.json())
      .catch((e: any) =>{
        return Observable.throw(e)
      } )
