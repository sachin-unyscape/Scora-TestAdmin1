...
@Component({
  ...
  styleUrls: ['./to-create.component.scss'],
  animations: [
    trigger('dialog1', [
      transition('void => *', [
...
export class ToCreateComponent implements OnInit {
  ...
  ItemSetList(){
    ...
    var headers = new Headers();
    headers.append('Authorization', 'Bearer ' + this.cookieService.get('_PTBA'));
    this.http.get(credentials.host +"/review_menu/" + this.cookieService.get('_PAOID') +'/' + 'to_create',{headers:headers})
    .map(res => res.json())
    .catch((e: any) =>{
      return Observable.throw(e)
    } )
  ...
  AddNewItems(){
    ...
    localStorage.setItem("_tmpURL", '/as-an-author/to-create');
  }
