...
import { itemInActive } from './item-Inactive';


@Component({
  selector: 'app-to-modify',
  templateUrl: './to-modify.component.html',
  styleUrls: ['./to-modify.component.scss'],
  animations: [
    trigger('dialog1', [
      transition('void => *', [
...
export class ToModifyComponent implements OnInit {
  ...
  ItemSetList(){
    ...
    var headers = new Headers();
    headers.append('Authorization', 'Bearer ' + this.cookieService.get('_PTBA'));
    this.http.get(credentials.host +"/review_menu/" + this.cookieService.get('_PAOID') +'/' + 'to_modify',{headers:headers})
    .map(res => res.json())
    .catch((e: any) =>{
      return Observable.throw(e)
    } )
  ...
  getItemsetDet(id,sectionid){
    if(this.authService.canActivate()){
      ...
      var headers = new Headers();
      headers.append('Authorization', 'Bearer ' + this.cookieService.get('_PTBA'));
      this.http.get(credentials.host + "/item_details/" +this.cookieService.get('_PAOID') +"/"+ id + "/all_sections/" + 2,{headers:headers})
      .map(res => res.json())
      .catch((e: any) =>{
        return Observable.throw(e)
      } )
  ...
  AddNewItems(){
    ...
    localStorage.setItem("_tmpURL", '/as-an-author/to-modify');
  }
