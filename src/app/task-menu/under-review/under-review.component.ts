...
import { updateItemScore } from '../../task-menu/to-create/updateItemScore';
import { section_status_update } from '../../task-menu/to-create/section_status_update';
import { itemInActive } from "../under-review/item-Inactive";
...
export class UnderReviewComponent implements OnInit {
  ...
  constructor(private http:Http,private router: Router,public route:ActivatedRoute,private dragulaService: DragulaService,private getItemService :GetItemService,private cookieService: CookieService,private authService :AuthServiceService,private modalService: BsModalService, private _notifications: NotificationsService,public sanitizer: DomSanitizer) {
    ...
    this.ItemInactive = new itemInActive();
  ...
  public ItemInactive :itemInActive;
  ...
  public ReplacedItemsetItemid;
  public Replaceditem_refID;
  public ReplacedItemId;
  public ReplacedItemType;
  ...
  ngOnInit() {
   this.diffLevel_boolean = true;
    ...
    this.route.params.subscribe(
        ...
        if(this.itemset_id != 0 && this.path == 13){
          this.ItemSetList();
        ...
        else if(this.itemset_id != 0 && this.path == 14){
          this.ItemSetList();
          this.viewAllItems(this.itemset_id,this.sectionid);
        }
        else{
  ...
  ItemSetList(){
    ...
    var headers = new Headers();
    headers.append('Authorization', 'Bearer ' + this.cookieService.get('_PTBA'));
    this.http.get(credentials.host + "/under_review_itemset/" + this.cookieService.get('_PAOID'),{headers : headers})
    .map(res => res.json())
    .catch((e: any) =>{
      return Observable.throw(e)
    } )
  ...
  getItemsetDet(id,sectionid){
    ...
    this.isclick = true;
      ...
      var headers = new Headers();
      headers.append('Authorization', 'Bearer ' + this.cookieService.get('_PTBA'));
      this.http.get(credentials.host + "/item_details/" +this.cookieService.get('_PAOID') +"/"+ id + "/all_sections/"+ 1+'/'+ 1,{headers:headers})
      .map(res => res.json())
      .catch((e: any) =>{
        return Observable.throw(e)
      } )
      ...
      .subscribe(
        data => {
          ...
          if(this.allItemListFlag){
            ...
            if(this.path == 3 &&  this.backToviewList == false){
  ...
  AddNewItems(){
    if(this.allItemListFlag){
      this.router.navigate(['Items/additem', this.choosenItemSetId, this.choosenItemSetSection,2]);
    }else{
      this.router.navigate(['Items/additem', this.choosenItemSetId, this.choosenItemSetSection,1]);
    }
  }
  ...
  getActiveItems(){
    ...
    var headers = new Headers();
    headers.append('Authorization', 'Bearer ' + this.cookieService.get('_PTBA'));
    this.http.get(credentials.host + "/item_details/" +this.cookieService.get('_PAOID') +"/"+ this.choosenItemSetId + "/all_sections/"+ 1+'/'+ 1,{headers:headers})
    .map(res => res.json())
    .catch((e: any) =>{
      return Observable.throw(e)
    } )
    ...
    .subscribe(
      data => {
        ...
        for(var n=0;n<this.getOneItem.itemset_sections.length;n++){
          // if(this.getOneItem.itemset_sections[i].section_id == this.choosenItemSetSection){
            index = n;
            var item_count = 1;
            this.eachSectionItems = this.getOneItem.itemset_sections[index];
            // this.itemsetComments = this.getOneItem.itemset_sections[index].comments;
          ...
          // }
          if(this.getOneItem.itemset_sections[n].section_id == this.choosenItemSetSection){
            var sectionIndex = n;
          }
        }
        this.eachSectionItems = this.getOneItem.itemset_sections[sectionIndex];
       ...
       this.selectedSectionItems = this.eachSectionItems;
  ...
  getItemsetRelatedItems(){
    ...
    var headers = new Headers();
    headers.append('Authorization', 'Bearer ' + this.cookieService.get('_PTBA'));
    this.http.get(credentials.host + "/itemset_related_items/" + this.cookieService.get('_PAOID') + '/' + this.choosenItemSetId + '/' + this.choosenItemSetSection +'/'+ this.ReplacedItemId +'-'+ this.ReplacedItemType,{headers:headers})
    .map(res => res.json())
    .catch((e: any) =>{
      return Observable.throw(e)
    } )
  ...
  AddFromItemBank(data){
    if(this.authService.canActivate()){
      if(this.eachSectionItems.access){
        this.searchedItems = [];
        this.searchItems = "";
        this.selectedSub=[];
        this.selectedTopic =[];
  ...
  searchByFilter(event){
    if(this.authService.canActivate()){
      if(this.searchItems == '' || this.searchItems == undefined){
        ...
        this.searchFilter.org_id = this.cookieService.get('_PAOID');
        this.searchFilter.reference = this.ReplacedItemId +'-'+ this.ReplacedItemType;
        this.showLoad = true;
        var body = this.searchFilter;
        var headers = new Headers();
        headers.append('Content-Type', 'application/json');
    ...
    this.itemMapDetail.items = [];
      for(var i=0;i<this.selectedSectionItems.section_items.length;i++){
        if(this.selectedSectionItems.section_items[i].item_type !=6){
          if(!this.itemidArray.includes(this.selectedSectionItems.section_items[i].item_id)){
  ...
  onItemDrop(data,sectionid){
    ...
    this.itemMapDetail.itemset_item_refer_id = this.Replaceditem_refID;
  ...
  insertItems(){
    if(this.authService.canActivate() && this.itemMapDetail.items.length !=0){
      ...
      if(this.itemMapDetail.items.length > 1 && this.ReplacedItemType != 6){         // restrict to map only one item to the section
        ...
        .subscribe(
          data => {
            ...
            if(data.success){
              ...
              this.saveMsg = false;
              if(this.allItemListFlag){
                for(var im=0;im<data.items.section_items.length;im++){
                  if(data.items.section_items[im].item_status != 'Approved'){
                    this.sectionStatus = true;
                    break;
                  }
                }
            ...
            // for(var s=0;s<this.getOneItem.itemset_sections.length;i++){
            //   if(this.getOneItem.itemset_sections[s].section_id == this.choosenItemSetSection){
            //     this.getOneItem.itemset_sections[s].section_items = this.eachSectionItems;
            //   }
            // }
            if(this.eachSectionItems.section_items.length != 0){
              this.EachItemComments = this.eachSectionItems.section_items[0].comments;
  ...
  // editItem(sectionid,editableItem){
  ...
  //         if(editableItem.answer_choices[a].choice_elements[c].answer_df_id == 6 || editableItem.answer_choices[a].choice_elements[c].answer_df_id == 10 || editableItem.answer_choices[a].choice_elements[c].answer_df_id == 11){
  ...
  //       if(editableItem.answer_choices[ma].answer_df_id == 6 || editableItem.answer_choices[ma].answer_df_id == 10 || editableItem.answer_choices[ma].answer_df_id == 11){
  ...
  //   this.router.navigate(['author/Items/additem', this.choosenItemSetId, sectionid,14]);

  // }

  editItemList(sectionid,editableItem){
    localStorage.setItem('directive_id', '0');
    localStorage.setItem('item_count', '0');
    if(this.eachSectionItems.access == true && editableItem.item_status != 'Approved'){
      if(editableItem.item_type != 6){
        for(var i=0;i<editableItem.item.length;i++){
      ...
      this.getItemService.getResponse(editableItem,this.getOneItem,'');
      this.router.navigate(['Items/additem', this.choosenItemSetId, sectionid,14]);
    }
  }
  ...
  UpdateItemScoreModal(template: TemplateRef<any>,itemscore,itemsetItemId,itemstatus){
    if(this.eachSectionItems.access == true && itemstatus != 'Approved'){
        ...
        data => {
          ...
          if(data.success == true){
            this._notifications.create('',data.message, 'info')
                ...
                //  this.router.navigate(['author/Tasks/ToReview'])
                this.showItemList = false;
                this.showItemSetList = true;
                this.showALLItems = false;
                this.ItemSetList();
  ...
  replaceItemPOPup(itemSetItemid,ref_id,openReplaceItemModal,itemstatus,item_id,itemType){
    if(this.eachSectionItems.access == true && itemstatus != 'Approved'){
      this.ReplacedItemsetItemid = itemSetItemid;
      this.Replaceditem_refID = ref_id;
      this.ReplacedItemId = item_id;
      this.ReplacedItemType = itemType;
      this.modalRef = this.modalService.show(openReplaceItemModal, Object.assign({}, { class: ' modal-md' },this.config));
    }
  }

  inactiveItem(flag){
    // this.ItemInactive.org_id = parseInt(this.cookieService.get('_PAOID'));
    // this.ItemInactive.itemset_item_id = this.ReplacedItemsetItemid;
    // this.ItemInactive.itemset_item_refer_id = this.Replaceditem_refID;


    // this.showLoad = true;
    // var body = this.ItemInactive;
    // var headers = new Headers();
    // headers.append('Content-Type', 'application/json');
    // headers.append('Authorization', 'Bearer ' + this.cookieService.get('_PTBA'));
    // return this.http.post(credentials.host + '/itemset_item_inactive', body,{headers: headers})
    // .map(res => res.json())
    // .catch((e: any) =>{
    //   return Observable.throw(e)
    // } )
    ...
    //       if(data.success == true){
           ...
           if(flag == 2){
            ...
            localStorage.setItem('directive_id', '0');
            localStorage.setItem('item_count', '0');
             this.getItemService.getReplaceItemDetails(this.ReplacedItemsetItemid,this.Replaceditem_refID);
            this.router.navigate(['Items/additem', this.choosenItemSetId, this.choosenItemSetSection,13]);
           }
    //       } else{
    //         this.modalRef.hide();
    //         this._notifications.create('',data.message, 'error')
