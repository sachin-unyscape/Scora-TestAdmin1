...
import { attributeSearch } from './attributefilter';
import { DragulaModule, DragulaService } from 'ng2-dragula/ng2-dragula';
import { itemMap } from './itemMap';
import { deleteObj } from './deleteItem-itemset';
...
import { updateItemScore } from './updateItemScore';
...
@Component({
  selector: 'app-view-item-sets',
  templateUrl: './view-item-sets.component.html',
  styleUrls: ['./view-item-sets.component.scss'],
  animations: [
    trigger('dialog1', [
      transition('void => *', [
...
export class ViewItemSetsComponent implements OnInit {
  ...
  public showAttributeFilterPopup = false;
  public subjectList=[];
  public topicsList = [];
  public subtopicList = [];
  public taxonomyList;
  ...
  ItemSetList(){
    if(this.authService.canActivate()){
      ...
      var headers = new Headers();
      headers.append('Authorization', 'Bearer ' + this.cookieService.get('_PTBA'));
      this.http.get(credentials.host + "/itemsets_list/" + this.cookieService.get('_PAOID'),{headers : headers})
      .map(res => res.json())
      .catch((e: any) =>{
        return Observable.throw(e)
      } )
  ...
  getItemsetDet(id,sectionid){
    if(this.authService.canActivate()){
      ...
      var headers = new Headers();
      headers.append('Authorization', 'Bearer ' + this.cookieService.get('_PTBA'));
      this.http.get(credentials.host + "/item_details/" +this.cookieService.get('_PAOID') +"/"+ id + "/all_sections/" + 8,{headers:headers})
      .map(res => res.json())
      .catch((e: any) =>{
        return Observable.throw(e)
      } )
      ...
      .subscribe(
        data => {
          this.itemSet_id = data.itemset_id;
    ...
    var temp_1=[];
         ...
         for (let data11 of data.itemset_sections[this.showItem]) {
            for (let data12 of data11.section_items) {

              if (data12.item_type === 1) {

                for (let data1 of data.itemset_sections) {
                  for (let data2 of data1.section_items) {
                    for (let data3 of data2.answer_choices) {
                      for (let data4 of data3.choice_elements) {
                        data3.data_insight = data4.data_insight
                        break
              ...
              if (data12.item_type === 2) {

                for (let data1 of data.itemset_sections) {
                  for (let data2 of data1.section_items) {
                    for (let data3 of data2.answer_choices) {
                      // for (let data4 of data3.choice_elements) {
                        data3.data_insight = data3.data_insight
                        break
          ...
          // this.getOneItem.is_itemset = false;
          var index;
          ...
          if(this.allItemListFlag){
            ...
            this.custom_Attributes = this.eachSectionItems.section_items[this.showItem];
  ...
  showQues(index){
    ...
    var temp=[];
        ...
        if (this.eachSectionItems.section_items[this.showItem].item_type === 1) {

          
            for (let data2 of this.eachSectionItems.section_items[this.showItem]) {
              for (let data3 of data2.answer_choices) {
                for (let data4 of data3.choice_elements) {
                  data3.data_insight = data4.data_insight
                  break
        ...
        if (this.eachSectionItems.section_items[this.showItem].item_type === 2) {
    ...
    console.log(this.eachSectionItems.section_items)
  ...
  AddNewItems(){
    ...
    localStorage.setItem("_tmpURL", '/ItemSets/viewitemsets');
  }
  ...
  filterWithAttributeses(){
    this.showAttributeFilterPopup = !this.showAttributeFilterPopup;
  }
  ...
  ApplyFilter(){
    this.showAttributeFilterPopup = false;
    ...
    this.searchFilter.search_data = this.searchItems;
        this.searchFilter.org_id = this.cookieService.get('_PAOID');
        this.searchFilter.itemset_id = this.itemSet_id;
        this.searchFilter.section_id = this.choosenItemSetSection;
        this.showLoad = true;
    ...
    let validateFilterValues:Boolean = false;

    if (
      this.searchFilter.filters.subject.length !== 0 ||
      this.searchFilter.filters.topic.length !== 0 ||
      this.searchFilter.filters.subtopic.length !== 0 ||
      this.searchFilter.filters.difficulty_level !== '' ||
      this.searchFilter.filters.taxonomy !== ''
      ) {
      validateFilterValues = true;
      // this.validateFilter = true;
    }
    if(validateFilterValues === false){
      this.showLoad = false;
      // this.validateFilter = false;
    }

    if (validateFilterValues === true) {
    this.showAttributeFilter = false;
  ...
  searchByFilter(event){
    ...
    if(this.authService.canActivate()){
      if((this.searchItems == '' || this.searchItems == undefined || !pattern1.test(this.searchItems))){
        ...
        let checkFilterAvailable;
        checkFilterAvailable = false;
        ...
        if (
          ...
          checkFilterAvailable = true;
        }

        if (checkFilterAvailable === false) {
          this.searchFilter.filters = new attributeSearch();
        }
  ...
  insertItems(){
    ...
    if(this.getOneItem.is_itemset == true){
      if(this.itemMapDetail.items.length > this.selectedSectionItems.remaining_no_of_items){
        this.itemsetMapAlert = "You have reached the limit for the maximum items allowed in this Item Set. If you'd like to add a new item, please delete an existing one.";
...
editItemList(sectionid,editableItem,editFlag){
  ...
  editItemset(itemsetId, val){
    if (val === true) {
      localStorage.removeItem("_IDVI-S");
      localStorage.setItem("_IDVI-S", itemsetId);
