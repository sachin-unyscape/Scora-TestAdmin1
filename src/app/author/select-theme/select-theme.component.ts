...
export class SelectThemeComponent implements OnInit {
  ...
  deleteIcon(){
    ...
    if (this.authService.canActivate()) {
      this.showload = true;
      let formData={
        file_path:this.tenantDetails.tenant_logo,
      ...
      this.http.post('http://15.207.209.163/new-scora/scoraauthor/public/api/file_delete',formData,{ headers: headers }).subscribe(data=>{
        this.showload = false;
        console.log(data);
        let response:any = data;
        let reponseBody=JSON.parse(response._body);
        if(reponseBody.success == true)
        this._notifications.create('',reponseBody.message, 'info', {timeOut: 3000});
        else
          this._notifications.info(reponseBody.message);
        this.getMetaData();
      },
      (error) => {
        this.showload = false;
  ...
  onImageUpload(event) {
    ...
    console.log("JJJJJJJJJJJJJJJJJJ",file.name)
  ...
  changeTheme(selectedTheme) {
     this.showload = true;
      let body= {} ;
      body["orgId"] = this.cookieService.get("_PAOID");
      ...
      return this.http
        .post(credentials.host + "/tenant_theme_first_update", body, {
          headers: headers,
        })
        .map((res) => res.json())
        .catch((e: any) => {
