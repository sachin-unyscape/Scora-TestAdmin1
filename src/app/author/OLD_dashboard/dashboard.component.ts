...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  constructor(private router: Router, private activeroute: ActivatedRoute ,private http:Http,private cookieService: CookieService,private authService :AuthServiceService,private GetItemService:GetItemService,private modalService: BsModalService) {
...
public chartTitle;
  ...
  ngOnInit() {
    ...
    // var alertBoolean = localStorage.getItem('showAlert');
    //
    // if(alertBoolean == null  || alertBoolean == undefined ){
    //   this.ExpireAlert = true;
    //   localStorage.setItem('showAlert', this.ExpireAlert);
    // }else{
  ...
  getChartDetailsTabOne()
  {
    ...
    data => {
          ...
          if(data.test_grp_performance.length != 0)
          {
            ...
            this.getChartDetailsChartTwo(this.tempTestId);
            this.getChartDetailsChartThree(this.tempTestId);
          }
          else
          {
  ...
  getChartDetailsChartTwo(testId)
  {
    this.paChart2 = false;
    this.showload = true;
    var headers = new Headers();
    ...
    this.http.get(credentials.userdashboardHost +'/pa_skill_grp_analysis/'+this.cookieService.get('_PAOID')+'/'+testId,{headers:headers})
    ...
    data => {
        ...
        this.showload = false;
        this.skillGrp = true;
        this.barChartLabelsTab2Two = [];
        this.barChartDataTab2Two = [];
        var tempMaxChartData;
        tempMaxChartData = [];


        this.barChartLabelsTab2Two = data.metadata;
        this.skillGrpTab2 = data.pa_skill_grp;
        tempMaxChartData.push(data.pa_skill_grp[0].OneLessthan30);
        tempMaxChartData.push(data.pa_skill_grp[0].Two30to40);
        tempMaxChartData.push(data.pa_skill_grp[0].Three40to50);
        tempMaxChartData.push(data.pa_skill_grp[0].Four50to60);
        tempMaxChartData.push(data.pa_skill_grp[0].Five60to70);
        tempMaxChartData.push(data.pa_skill_grp[0].Six70to80);
        tempMaxChartData.push(data.pa_skill_grp[0].Seven80to90);
        tempMaxChartData.push(data.pa_skill_grp[0].EightGreaterthan90);

        this.barChartDataTab2Two = [
        {data: tempMaxChartData, label:'No.of Users'}
        ];
  ...
  getChartDetailsChartThree(testId)
  {
    this.paChart3 = false;
    this.showload = true;
    var headers = new Headers();
  ...
  testSelectionDropDown(curTestId)
  {
    ...
    this.getChartDetailsChartTwo(curTestId);
    this.getChartDetailsChartThree(curTestId);
 ...
 itemsByAuthorHome()
 {
 this.itemsByAuthorDrill = false;
 }

 itemsByItemTypeHome()
 {
 this.itemsByItemTypeDrill = false;
 }

 itemsByAuthorAndItemType()
  {
    this.IBRChart2 = false;
      this.showload= true;
    this.itemsByAuthorDrill = false;
    this.itemsByItemTypeDrill = false;
    var headers = new Headers();
    ...
    this.http.get(credentials.userdashboardHost+'/item_description_author_itemtype/'+this.cookieService.get('_PAOID'),{headers:headers})
    ...
    data => {
      ...
      this.showload= false;
      this.itemsByAuthorAndItemTypeResponse = data;

      if(this.itemsByAuthorAndItemTypeResponse.item_distribution_by_author.length != 0)
      {
      this.itemsByAuthorLabels = [];
      this.itemsByAuthorData = [];
      for(var f=0;f<this.itemsByAuthorAndItemTypeResponse.item_distribution_by_author.length;f++)
      {
      this.itemsByAuthorLabels.push(this.itemsByAuthorAndItemTypeResponse.item_distribution_by_author[f].Tenant_user_nm);
      this.itemsByAuthorData.push(this.itemsByAuthorAndItemTypeResponse.item_distribution_by_author[f].No_of_items);
      }
      }

      if(this.itemsByAuthorAndItemTypeResponse.item_distribution_by_itemtype.length != 0)
      {
      this.itemsByItemTypeData = [];
      this.itemsByItemTypeLabels = [];
      for(var f=0;f<this.itemsByAuthorAndItemTypeResponse.item_distribution_by_itemtype.length;f++)
      {
      this.itemsByItemTypeLabels.push(this.itemsByAuthorAndItemTypeResponse.item_distribution_by_itemtype[f].Item_Type_nm);
      this.itemsByItemTypeData.push(this.itemsByAuthorAndItemTypeResponse.item_distribution_by_itemtype[f].No_of_items);
      }
      }


      this.itemBankDet = true;
    },
    error => {
      ...
      data => {
        ...
        if(this.subjectWiseItemDetailsData.items_by_sub_topic.length != 0)
        {
          ...
          for(var w=0;w<this.subjectWiseItemDetailsData.items_by_sub_topic.length;w++)
          {
            ...
            // tempSubTopicArray.push(this.subjectWiseItemDetailsData.items_by_sub_topic[w].No_of_Items);
          }

          //   this.itemsBySubTopicData = [
          //   {data: tempSubTopicArray, label:'No.of Items'}
          // ];
